...
Note that for controlled-access data, a GDC [authentication token](#authentication) is required. Using the `BiocParallel` package may be useful for downloading in parallel, particularly for large numbers of smallish files.
...
```{r bulkDL}
mfile = tempfile()
write.table(manifest_df[1:50,],mfile,col.names=TRUE, row.names=FALSE, quote=FALSE,sep="\t")
