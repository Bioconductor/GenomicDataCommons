...
This code block downloads the `r nrow(ge_manifest)` gene expression files specified in the query above. Using multiple processes to do the download very significantly speeds up the transfer in many cases.  On a standard 1Gb connection, the following completes in about 30 seconds.
...
In the case of the `projects` entity, the default results can be viewed as a `data.frame`.

```{r simplifyProjects}
head(data.frame(presults))
```
