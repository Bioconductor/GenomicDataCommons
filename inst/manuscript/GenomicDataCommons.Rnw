...
molecular biology data \parencite{Huber2015}. Based on the statistical
...
via an R client interface. The GDC API can be roughly divided into
metadata query capabilities and data download capabilities. The
GenomicDataCommons package is implemented as a set of S3 classes and
methods with accompanying utility functions.
...
approach, XXXXXX, similar in spirit to that used by packages such as
