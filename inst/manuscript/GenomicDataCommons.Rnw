...
\begin{abstract}
  ...
  infrastructure.  The \Bioconductor{} project \parencite{Huber2015}
  ...
  expect that \Bioconductor{} developer and bioinformatics community
  ...
  \textbf{Availability}:
  \url{https://bioconductor.org/packages/GenomicDataCommons} \& \url{https://github.com/Bioconductor/GenomicDataCommons}.
...
data repositories, while in addtion providing data that have been
`harmonized' using stardard, publicly-available pipelines\footnote{See
  \url{https://docs.gdc.cancer.gov/Data/Introduction/} for
  details.}. An additional and unusual feature of the GDC is that
users can deposit and then apply standard GDC pipelines to their own
genomic data.
