...
res = tibble::tibble(Collection=as.character(ent),
                 `Data Categories Available`=sapply(ent, function(x) length(available_expand(x))),
                 `Total Field Count`=sapply(ent, function(x) length(available_fields(x))),
                 )
knitr::kable(res, caption="The Genomic Data Commons API includes separate collections for each of the rows listed. The number of data types and available fields are given in columns two and three, respectively.", label = "tab:avail", row.names=FALSE, booktabs=TRUE)
@ 
...
The \code{transfer()} function will use the GDC transfer client (a
separate command-line tool) to perform robust, performant download of
the data.
...
proides a large, innovative community of researchers and software
