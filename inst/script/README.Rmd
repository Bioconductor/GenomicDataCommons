...
filterAllNA <- function(df) {
    df[] <- Filter(function(x) !all(is.na(x)), as.list(df))
    df
}
...
bindrowname <- function(resultList) {
    ...
    dataDF <- S4Vectors::DataFrame(datadf)
    rownames(dataDF) <- rep(
        names(resultList), vapply(resultList, nrow, integer(1L))
    )
    as.data.frame(filterAllNA(dataDF))
}
```
