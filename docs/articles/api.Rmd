...
```{r files10}
a = files()
a %>% filter(~data_format=='VCF') %>% response() %>% str()
resp = a %>% facet('data_format') %>% response()
head(resp$aggregations$data_format)
```
...
head(resp$aggregations[[1]])
head(resp$aggregations$project.program.name)
```
...
```{r annotations10}
a = annotations()
resp = a %>% select(available_fields(a)) %>% response()
resp = a %>% response()
str(resp$results[[1]])
```
...
```{r}
manifest_path = manifest(uuids = file_ids)
manifest_path
```
